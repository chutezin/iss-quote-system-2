 ...
 *
 * @class 	ISS_RQAQ_Session
 * @package iss quote system 
 * @since   1.0.0
 * @author  chutes
 */
class ISS_RQAQ_Session extends WC_Session {
    ...
    public function __construct() {
        $this->_cookie = 'ISS_RQAQ_Session_' . COOKIEHASH;
        ...
        if ( $cookie = $this->get_session_cookie() ) {
            ...
            if ( time() > $this->_session_expiring ) {
                ...
                $session_expiry_option = '_ISS_RQAQ_Session_expires_' . $this->_customer_id;
    ...
    public function set_customer_session_cookie( $set ) {
        if ( $set ) {
            ...
            // Set the cookie
            if ( ! headers_sent() ) {
            wc_setcookie( $this->_cookie, $cookie_value, $this->_session_expiration, apply_filters( 'ISS_RQAQ_Session_use_secure_cookie', false ) );
            }
        }
    }
    ...
    public function set_session_expiration() {
        $this->_session_expiring    = time() + intval( apply_filters( 'ISS_RQAQ_Session_expiring', 60 * 60 * 47 ) ); // 47 Hours
        $this->_session_expiration  = time() + intval( apply_filters( 'ISS_RQAQ_Session_expiration', 60 * 60 * 48 ) ); // 48 Hours
    }
    ...
    public function get_session_data() {
        return $this->has_session() ? (array) get_option( '_ISS_RQAQ_Session_' . $this->_customer_id, array() ) : array();
    }
    ...
    public function save_data() {
        ...
        if ( $this->_dirty && $this->has_session() ) {
            ...
            $session_option        = '_ISS_RQAQ_Session_' . $this->_customer_id;
            $session_expiry_option = '_ISS_RQAQ_Session_expires_' . $this->_customer_id;
    ...
    public function destroy_session() {
        // Clear cookie
        wc_setcookie( $this->_cookie, '', time() - YEAR_IN_SECONDS, apply_filters( 'ISS_RQAQ_Session_use_secure_cookie', false ) );
    ...
    public function cleanup_sessions() {
        ...
        if ( ! defined( 'WP_SETUP_CONFIG' ) && ! defined( 'WP_INSTALLING' ) ) {
            ...
            foreach ( $wc_session_expires as $option_name ) {
                ...
                $expired_sessions[] = "_ISS_RQAQ_Session_$session_id"; // Session key
            }
